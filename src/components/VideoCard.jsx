...
const VideoCard = ({displayData,setDeleteVideoResponse,insideCategory}) => {
  ...
  const handleShow = async() => {
    setShow(true);
    // call savehistoryapi
    // video details tobe stored in history
...
const videoDragStart=(e,videoId)=>{
  // console.log(`video with  id ${videoId} is dragged`); 
  ...
  return (
    ...
    <Card draggable={true} onDragStart={e=>videoDragStart(e,displayData?.id)} style={{ height: '250px' }}>
      ...
      <Card.Body>         
        <Card.Text className='d-flex justify-content-between'>
          ...
          {/* arrow function bcs this function has a argument*/}
    ...
    <Modal size='lg' centered show={show} onHide={handleClose}>
        ...
        <Modal.Body>
        <iframe width="100%" height="410" src={`${displayData?.link}?autoplay=1`} title="Caption"  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
        </Modal.Body>
